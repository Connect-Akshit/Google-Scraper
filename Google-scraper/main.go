...
var googleDomains = map[string]string{
	"com": "https://wwww.google.com/search?q=",
...
func randomUserAgent() string {
	...
	randNum := rand.Int() % len(userAgents) //Generating random numbers between 1 and the no. of users that's why am taking length of userAgents.
...
func main() {
	res, err := GoogleScrape("Barcelona", "com", "en", nil, 1, 30, 10) //calling the google function and posting the text(barcelona:--this is the text we will search on google) in the function
	if err == nil {
		for _, res := range res { //if there is no error we will range over response.
			fmt.Println(res)
		}
	}
}
